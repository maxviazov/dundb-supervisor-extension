(function (global) {
  ...
  function findMetricBox(titleEl, titleText) {
    ...
    const maxLen = titleText === "סקור ענפי" ? 280 : 180;
    ...
    for (let depth = 0; depth < 10 && box; depth++, box = box.parentElement) {
      ...
      if (boxText.length > maxLen) continue;
      return box;
    }
  ...
  function extractMetricBoxes(root) {
    ...
    for (const [key, title] of Object.entries(titleMap)) {
      ...
      if (key === "score") {
        ...
        const nums = withoutNotes.match(/\b(\d{1,3})\b/g) || [];
  ...
  function findIndustryScoreNumber(root) {
    ...
    if (titleEl) {
      const box = findMetricBox(titleEl, "סקור ענפי");
      const fromLarge = findLargeNumberInBox(box, 10);
